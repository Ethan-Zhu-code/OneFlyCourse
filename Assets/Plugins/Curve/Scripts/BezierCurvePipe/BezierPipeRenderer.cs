...
public class BezierPipeRenderer : MonoBehaviour
{
    ...
    public void PreviewMesh()
    {
        ...
        if (preview)
        {
            ...
            if (!Application.isPlaying)
            {
               DestroyImmediate(tempCache);
            }
            else
            {
                Destroy(tempCache);
            }
            tempCache = MeshExtruder.Extrude(path, GetShapeArray(), u, fixedTotalV, vFactor);
            previewMeshFilter.mesh = tempCache;
